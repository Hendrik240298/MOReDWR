...
print(
    f"Primal soluion {np.reshape(primal_solution, (n_dofs['space'], -1)).shape}")

# test estimate for different primal POD qualities
pod_basis, S, _ = scipy.linalg.svd(np.reshape(
    primal_solution, (n_dofs['space'], -1)), full_matrices=False)
pod_basis_dual, S, _ = scipy.linalg.svd(np.reshape(
    dual_solution, (n_dofs['space'], -1)), full_matrices=False)

# pod_basis = pod_basis[:, :20]
# pod_basis_dual = pod_basis_dual[:, :20]
...
print(f"spatial dofs: {pod_basis.shape}")
probes = 5
...
for i_cut, cut in enumerate(cutting):
    for j_cut, cut_dual in enumerate(cutting_dual):
        ...
        if np.sum(true_tol) == np.sum(esti_tol):
            ...
            # prediction.append(''.join(np.array2string(
            #     np.array([0, 0, 0, n_slabs]))))
            # prediction.append(f"{0}|{0}|{0}|{n_slabs}")
            prediction[i_cut, j_cut] = 100
            ...
            # prediction.append(f"{eltg}|{egtl}|{egtg}|{eltl}")
            prediction[i_cut, j_cut] = (
                eltl) / (eltg + egtl + egtg + eltl)*100
            # np.array2string(
            # np.array([eltg, egtl, egtg, eltl]), separator='/'))
            # print(
