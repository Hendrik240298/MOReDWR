...
print("n_dofs =", n_dofs)
print("slab_properties =", slab_properties)
print("index2measuredisp =", index2measuredisp)
#print("dof_matrix =", dof_matrix)
#print("grid =", grid)
...
for i in range(slab_properties["n_total"]):
    ...
    # NEW ERROR ESTIMATOR BASED ON REDUCED LINEAR EQUATION SYSTEM
    # hard code cG(2) in error estimator
    slab_u = np.hstack(projected_reduced_solutions["value"][-2:]).T
    z_shift = -1 # 0
    slab_z = np.hstack(dual_solutions["value"][i*2+1+z_shift : i*2+3+z_shift]).T
    residual = - D.dot(slab_u) - C.dot(projected_reduced_solutions["value"][-3]) + primal_system_rhs[i][n_dofs["time_step"]:].copy()
    temporal_interval_error.append(residual.dot(slab_z))
