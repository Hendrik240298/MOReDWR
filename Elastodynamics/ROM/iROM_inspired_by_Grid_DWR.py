...
for i in list(range(slab_properties["n_total"]-10, slab_properties["n_total"]))[::-1]:
    if i == len(list(range(slab_properties["n_total"])))-1:
        ...
        ) + mass_matrix_down_left.dot(np.zeros(dual_matrix.shape[0]))
    else:
        dual_rhs = dual_system_rhs[i].copy(
...
for i, dual_solution_slab in enumerate(dual_solutions_slab["value"]):
    for j in range(len(slab_properties["time_points"][i][1:])):
        ...
        dual_solutions["value"].append(
            dual_solutions_slab["value"][i][range_start:range_end])
...
reduced_mass_matrix_up_right_no_bc_estimator = reduce_matrix(
    mass_matrix_up_right_no_bc, pod_basis_dual, pod_basis)
# ------------------------------------------------------------------------------------------
# %% Primal ROM solve
...
# slab_properties["n_time_unknowns"]  #1# slab_properties["n_time_unknowns"]
bunch_size = 1
...
for it_bucket in range(nb_buckets):
    ...
    while (True):
        ...
        # ----------------------------------------- ROM solves -----------------------------------------
        start_time = time.time()
        # [np.zeros(system_matrix_reduced.shape[0])]
        ...
        dual_reduced_solutions = [
            np.zeros(mass_matrix_down_left_reduced.shape[0])]  # not really reduced
        for i in range(len_block_evaluation):
        ...
        for i in range(len_block_evaluation):
            if i > 0:
                residual_slab = - reduced_matrix_no_bc_estimator.dot(primal_reduced_solutions[i]) + reduce_vector(
                    rhs_no_bc[i+bucket_shift].copy(), pod_basis_dual) + reduced_mass_matrix_up_right_no_bc_estimator.dot(primal_reduced_solutions[i-1])
            else:
                residual_slab = - reduced_matrix_no_bc_estimator.dot(primal_reduced_solutions[i]) + reduce_vector(rhs_no_bc[i+bucket_shift].copy(
                ), pod_basis_dual) + reduce_vector(mass_matrix_up_right_no_bc.dot(last_bucket_end_solution), pod_basis_dual)
            temporal_interval_error.append(
                dual_reduced_solutions[i].dot(residual_slab))
            goal_func_on_slab.append(primal_reduced_solutions[i].dot(
                reduce_vector(dual_rhs_no_bc[i], pod_basis)))
            if np.abs(temporal_interval_error[i] + goal_func_on_slab[i]) > 1e-12:
                temporal_interval_error_relative.append(
                    temporal_interval_error[i]/(temporal_interval_error[i] + goal_func_on_slab[i]))
            else:
        ...
        if estimated_error < tol_rel:
            ...
            if index_primal < len_block_evaluation-1:
                # projected_reduced_solutions[index_primal - 1]n
                old_projected_dual_solution = project_vector(
            ...
            for k in range(slab_properties["n_time_unknowns"]):
                primal_solution = new_projection_solution[k *
                                                          n_dofs["time_step"]:(k+1)*n_dofs["time_step"]]
                pod_basis["displacement"], bunch["displacement"], singular_values["displacement"], total_energy["displacement"] \
                    = iPOD(pod_basis["displacement"],
                           bunch["displacement"],
                           singular_values["displacement"],
            ...
            for k in range(slab_properties["n_time_unknowns"]):
                dual_solution = new_projection_dual_solution[k *
                                                             n_dofs["time_step"]:(k+1)*n_dofs["time_step"]]
                pod_basis_dual["displacement"], bunch_dual["displacement"], singular_values_dual["displacement"], total_energy_dual["displacement"] \
                    = iPOD(pod_basis_dual["displacement"],
                           bunch_dual["displacement"],
    ...
    if False:
        ...
        last_bucket_end_solution = scipy.sparse.linalg.spsolve(
            primal_matrix, primal_rhs)
        number_FOM_solves += 1
    else:
        last_bucket_end_solution = project_vector(
            primal_reduced_solutions[index_primal], pod_basis)

    record_basis_size.append(
        pod_basis["displacement"].shape[1] + pod_basis["velocity"].shape[1])
    record_dual_basis_size.append(
        pod_basis_dual["displacement"].shape[1] + pod_basis_dual["velocity"].shape[1])
# %% ----------------------------------------- VERIFICATION -----------------------------------------
start_time = time.time()
last_bucket_end_solution = np.zeros(matrix_no_bc.shape[0])
...
for i in range(slab_properties["n_total"]):
    if i > 0:
        residual_slab = - reduced_matrix_no_bc_estimator.dot(primal_reduced_solutions[i]) + reduce_vector(
            rhs_no_bc[i].copy(), pod_basis_dual) + reduced_mass_matrix_up_right_no_bc_estimator.dot(primal_reduced_solutions[i-1])
    else:
        residual_slab = - reduced_matrix_no_bc_estimator.dot(primal_reduced_solutions[i]) + reduce_vector(rhs_no_bc[i].copy(
        ), pod_basis_dual) + reduce_vector(mass_matrix_up_right_no_bc.dot(last_bucket_end_solution), pod_basis_dual)
    temporal_interval_error.append(
        dual_reduced_solutions[i].dot(residual_slab))
    J_r_t[i] = primal_reduced_solutions[i].dot(
        reduce_vector(dual_rhs_no_bc[i], pod_basis))
    if np.abs(temporal_interval_error[i] + J_r_t[i]) > 1e-12:
        temporal_interval_error_relative.append(
            temporal_interval_error[i]/(temporal_interval_error[i] + J_r_t[i]))
    else:
        temporal_interval_error_relative.append(0)
estimated_error = np.max(np.abs(temporal_interval_error_relative))
...
print("speedup: act/max:     " + str(time_FOM/time_iROM) +
      " / " + str(slab_properties["n_total"]/number_FOM_solves))
print("Size ROM: u/v         " +
      str(pod_basis["displacement"].shape[1]) + " / " + str(pod_basis["velocity"].shape[1]))
print("Size ROM - dual: u/v  " +
      str(pod_basis_dual["displacement"].shape[1]) + " / " + str(pod_basis_dual["velocity"].shape[1]))
...
print("|J(u_h) - J(u_r)|/|J(u_h)| =",
      np.abs(np.sum(J_h_t) - np.sum(J_r_t))/np.abs(np.sum(J_h_t)))
...
if np.sum(true_tol) == np.sum(esti_tol):
    ...
    print(
        f"(error > tol & esti < tol): {eltg} ({round(100 * eltg / slab_properties['n_total'],1)} %)  (very bad)")
    print(
        f"(error < tol & esti > tol): {egtl} ({round(100 * egtl / slab_properties['n_total'],1)} %)  (bad)")
    print(
        f"(error > tol & esti > tol): {egtg} ({round(100 * egtg / slab_properties['n_total'],1)} %)  (good)")
    print(
        f"(error < tol & esti < tol): {eltl} ({round(100 * eltl / slab_properties['n_total'],1)} %)  (good)")
...
if PLOTTING:
    time_step_size = primal_solutions_slab["time"][-1][-1] / \
        (slab_properties["n_total"])
    ...
    plt.plot(np.arange(0, slab_properties["n_total"]*time_step_size, slab_properties["n_total"]
             * time_step_size/len(record_basis_size)), record_basis_size, c='#1f77b4', label="primal")
    # plt.grid()
    plt.plot(np.arange(0, slab_properties["n_total"]*time_step_size, slab_properties["n_total"]
             * time_step_size/len(record_dual_basis_size)), record_dual_basis_size, c='r', label="dual")
    ...
    # plt.yscale("log")
    plt.savefig("images/" + prefix_plot +
                "basis_development.eps", format='eps')
    plt.savefig("images/" + prefix_plot +
                "basis_development.png", format='png')
    plt.show()
